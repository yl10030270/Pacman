...
import ca.bcit.cst.comp2526.assign6.solution.entityfinder.DefaultEntityFinder;
import ca.bcit.cst.comp2526.assign6.solution.entityfinder.EntityFinder;
import ca.bcit.cst.comp2526.assign6.solution.entityfinder.EntityKeyMatcher;
...
import ca.bcit.cst.comp2526.assign6.solution.game.AbstractGame;
    ...
    implements PacManGame
{
     ...
     * power pellets.
     */
    private final List<PowerPelletEntity> powerPellets;

    /**
     * previous location.
     */
    private Location previousPacManLocation;
               ...
               FoundTooFewEntitiesException
    {
        ...
        powerPellets = findPowerPellets();
        previousPacManLocation = pacman.getLocation();
    }
    ...
    /**
     * find all power pellets.
     *
     * @return list of power entity.
     *
     * @throws FoundTooManyEntitiesException - too many entity.
     * @throws FoundTooFewEntitiesException  - too few entity.
     */
    private List<PowerPelletEntity> findPowerPellets()
        throws FoundTooManyEntitiesException,
               FoundTooFewEntitiesException
    {
        final List<PowerPelletEntity> entities;

        entities = findMultipleEntitiesMustExist(factory.getKeyForPowerPelletEntity(),
                                                 Integer.parseInt("4"));
    ...
    {
        final Location pacmanLocation;

        pacmanLocation = pacman.getLocation();

        if(pacmanLocation.equals(previousPacManLocation))
        {
            pacman.setCurrentDirection(Direction.NONE);
            updater.addUpdateLocation(pacman.getLocation());
        }
        else
        {
            previousPacManLocation = pacmanLocation;
        }

        movePacMan(updater);
        moveGhosts(updater);
        blinkPowerPellets(updater);
    }

    /**
     * blink power pellets.
     *
     * @param updater - world updater.
     */
    private void blinkPowerPellets(final WorldUpdater updater)
    {
        for(final PowerPelletEntity powerPellet : powerPellets)
        {
            final Location location;

            location = powerPellet.getLocation();

            if(location != null)
            {
                updater.addUpdateLocation(location);
            }
        }
    }
